  ...
  public async processOrderRequest(
    ...
    try {
      ...
      const completion = await openaiClient.chat.completions.create({
        ...
        tool_choice: "auto", // Lets OpenAI decide which tools to use
      ...
      if (message.tool_calls && message.tool_calls.length > 0) {
        console.log(
          `OpenAI wants to call ${message.tool_calls.length} function(s)`
        );
        ...
        for (const funcResult of functionResults) {
          if (
            funcResult.functionName === "check_inventory" &&
            funcResult.result.success
          ) {
            orderSummary = funcResult.result.order_summary;
          }
          if (
            funcResult.functionName === "generate_email" &&
            funcResult.result.success
          ) {
        ...
        if (orderSummary) {
          if (orderSummary.status === "available") {
            nextActions = [
              'Reply "CONFIRM ORDER" to place this order',
              "Ask questions about shipping or returns",
            ];
          } else if (orderSummary.status === "partial") {
            nextActions = [
              'Reply "CONFIRM PARTIAL" to order available items only',
        ...
        const directResponse =
          message.content || "I'm here to help with your toy store needs!";
