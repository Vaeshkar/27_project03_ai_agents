    ...
    switch (info_type) {
      case "hours":
        ...
        response = `Store hours: ${
          (storeInfo as any).opening_hours || "Monday to Friday: 9:00 - 18:00"
        }`;
        break;
      case "contact":
      ...
      case "returns":
        response =
          "Returns: 14-day return policy, items must be in original packaging";
        break;
      default:
        response = `${storeInfo.name} - ${storeInfo.location}. Phone: ${
          storeInfo.phone
        }. ${(storeInfo as any).opening_hours || "Open Monday-Friday 9-18"}`;
    }
    ...
    switch (action) {
      case "reserve":
        if (!order_summary) {
          return {
            success: false,
            message: "Order summary required for reservation",
          };
        }
      ...
      case "cancel":
        if (!order_summary) {
          return {
            success: false,
            message: "Order summary required for cancellation",
          };
        }
